...
function XML_Serializer() {this.contents = []; this.media = []; this.root = {};
this.isCollectingMedia = false; this.isExportingBlocksLibrary = false;};
...
function SnapSerializer() {this.init();};
...
SnapSerializer.prototype.loadProjectModel = function (xmlNode, ide, remixID) {
    ...
    var appInfo = xmlNode.attributes.app,
        ...
        extensionsModel = xmlNode.childNamed('extensions'),
        project = new Project;
    ...
    if (extensionsModel) {
        Process.prototype.parseCSV(extensionsModel.contents).asArray(
            ).forEach(ext => {ide[IDE_Morph.prototype.featureExtensions[
            IDE_Morph.prototype.featureExtensions.map(ext => ext[0]
            ).indexOf(ext)][1]](true);
        });
...
SnapSerializer.prototype.loadBlocksModel = function (model, targetStage) {
    ...
    var stage; this.scene = new Scene;
    ...
    };  this.objects = {}; stage.globalBlocks.forEach(def => def.receiver = null);
    this.objects = {}; this.scene = new Scene; this.mediaDict = {}; return {
        global : stage.globalBlocks, local : stage.customBlocks};};
...
SnapSerializer.prototype.loadSprites = function (xmlString, ide) {
    ...
    if (+model.attributes.version > this.version) {
        ...
        }; scene.stage.add(sprite); ide.sprites.add(sprite);
        sprite.scale = parseFloat(model.attributes.scale || '1');
        sprite.rotationStyle = parseFloat(
        model.attributes.rotation || '1');
        sprite.setDraggableTo(model.attributes.draggable !== 'false'
        ); sprite.isVisible = model.attributes.hidden !== 'true';
        sprite.heading = parseFloat(model.attributes.heading) || 0;
        sprite.gotoXY(+model.attributes.x || 0, +((model.attributes
        ).y) || 0); this.loadObject(sprite, model); (sprite
        ).fixLayout(); sprite.pauseGenericHatBlocks();});
    ...
    scene.stage.children.forEach(sprite => {
        ...
        if (sprite.inheritanceInfo) { // only sprites can inherit
            exemplar = scene.spritesDict[
                sprite.inheritanceInfo.exemplar
            ];  if (exemplar) {sprite.setExemplar(exemplar);};
        };  if (sprite.nestingInfo) { // only sprites may have nesting info
            anchor = scene.spritesDict[sprite.nestingInfo.anchor];
            if (anchor) {
                anchor.attachPart(sprite);
            };  sprite.rotatesWithAnchor = (
            sprite.nestingInfo.synch === 'true'
    );};}   ); (scene.stage
    ).children.forEach(sprite => {
        delete sprite.inheritanceInfo;
        if (sprite.nestingInfo) {
            sprite.nestingScale = +(
            (sprite.nestingInfo.scale
            ) || sprite.scale);
            delete sprite.nestingInfo;
        };
    });  this.objects = {};
...
SnapSerializer.prototype.loadCustomBlocks = function (
    ...
    element.children.forEach(child => {
                ...
                definition.declarations.set(
                    ...
                    [
                        ...
                        contains(['%b', '%boolUE'], child.attributes.type) ?
                            (child.contents ? asABool(child.contents) : null)
                                : child.contents,
                        options ? options.contents : undefined,
                        asABool(child.attributes.readonly),
                        asABool(isNil(child.attributes.irreplaceable) ?  child.attributes.static : child.attributes.irreplaceable),
        ...
        cap = child.childNamed('cap');
        if (cap) {
            definition.isCap = asABool(cap.contents);
        }

        special = child.childNamed('special');
        if (special) {
            definition.isSpecialReporter = asABool(special.contents);
        }

        specialType = child.childNamed('specialType');
        if (specialType) {
            definition.specialBlockType = specialType.contents;
        }
    ...
    if (model.tag === 'block') {
        if (Object.prototype.hasOwnProperty.call(
            ...
            migration = SpriteMorph.prototype.blockMigrations[model.attributes.s
                ...
                migrateToVariadic = migration.variadic;
                migrateInverse = migration.isInverse;
                skip = migration.skippedInput;
            };
        };
        ...
        if (child.tag === 'variables') {
            ...
            if (migrateToVariadic) {
                ...
                this.loadInput(
                    child,
                    ((inputs.length > 0) ? inputs[0].inputs()[i] : null),
                ...
                this.loadInput(
                    child,
                    inputs[(isNil(migration) ? i : (migrateInverse ? (inputs.length - i - 1) : i)) + migrationOffset],
                    block,
                    object
                );
            };
        }; i++; j++; if (!isNil(skip)) {if (j == skip) {i++;};};
    };  block.cachedInputs = null;  return block;
};

SnapSerializer.prototype.obsoleteBlock = function (isReporter, isCustom, selector) {var block = (isReporter ? new ReporterBlockMorph : new CommandBlockMorph); block.selector = (
isReporter ? 'reporterObsolete' : 'commandObsolete'); block.category = 'undefined'; if (isCustom) {block.setSpec('undefined');} else {block.setSpec(('undefined\, the selector \"'
).concat(selector, '\" isn\'t found!'));}; block.isDraggable = true; return block;}; /* To catch selectors or simply just setting the undefined or obsolete block without inputs. */

SnapSerializer.prototype.loadInput = function (model, input, block, object) {var inp, val; if (isNil(input)) {return;};
    if (model.tag === 'instruction' && model.children[0]) {
        if (model.children[0].tag === 'script') {
        inp = this.loadScript(model.children[0], object);
        } else if ((model.children[0].tag === 'block') || (model.children[0].tag === 'custom-block')) {
        inp = this.loadBlock(model.children[0], true, object);
        }; if (inp) {input.reactToDropOf(inp);}; inp.fixBlockColor();
    } else if (model.tag === 'script') {
        inp = this.loadScript(model, object);
        if (inp) {input.add(inp
        ); input.fixLayout();};
    ...
    } else if (contains(['list', 'listHidden'], model.tag)) {
        ...
        if (model.tag === 'listHidden') {
            if (input.hidden) {if (input.hiddenInput) {
            input.hideThatInput();};};
        }; input.fixLayout();
    } else if (model.tag === 'block' || model.tag === 'custom-block') {
        if (contains(SyntaxElementMorph.prototype.listOfContractives, input.slotSpec)) {
            input.replaceInput(
                input.inputs()[0],
    ...
    } else if (model.tag === 'textInput') {
        if (!((model.contents).toString(
        ) === input.label.text)) {
        input.toggleText();};
    } else {
...
SnapSerializer.prototype.loadValue = function (model, object) {
    ...
    case 'sprite':
        ...
        v.setDraggableTo(model.attributes.draggable !== 'false'
...
SnapSerializer.prototype.loadColor = function (colorString) {return new Color(colorString.split(',')[0], colorString.split(',')[1], colorString.split(',')[2], colorString.split(',')[3]);};
...
Project.prototype.toXML = function (serializer) {
    ...
    return serializer.format(
        '<project name="@" app="@" version="@">' +
            ...
            '</project>',
        (this.name || localize('Untitled')), serializer.app,
        serializer.version, (this.notes || ''), thumbdata,
        (this.scenes.asArray().indexOf(this.currentScene
        ) + 1), serializer.store(this.scenes.itemsArray()),
        IDE_Morph.prototype.whiteExtensionsList.toString()
    );
};
        ...
        Array.from(this.declarations.keys()).reduce((xml, decl) => {
            // to be refactored now that we've moved to ES6 Map:
                return xml + serializer.format(
                    ...
                    (' ').concat('static\= \"', asABool(this.declarations.get(decl)[4]).toString(), '\"'),
                    (' ').concat('readonly\= \"', asABool(this.declarations.get(decl)[3]).toString(), '\"'),
                    (this.declarations.get(decl)[5] ? 'separator\= \"' + this.declarations.get(decl)[5] + '\"' : ''),
...
BlockSlotMorph.prototype.toXML = function (serializer) {
    var block = this.evaluate();
...
Context.prototype.toXML = function (serializer) {
    if (this.isContinuation) {return '';
        ...
        this.inputs.reduce(
                (xml, input) => {
                    return xml + (input.toXML ? serializer.format('<input>$</input>', input) : '');
                },
                ''
            ),
