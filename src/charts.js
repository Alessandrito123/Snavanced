...
GridMorph.prototype.init = function (x, y, xLines, yLines) {
	...
	this.lines = [
		[190, 160, 40, 120, 30, 70, 82, 130, 163, 149, 138, 112, 186, 152, 161],
...
GridMorph.prototype.render = function (context) {
	// initialize my surface property
	var i, x, y, temp;

	temp = this.color;
	this.color = this.backgroundColor;
	GridMorph.uber.render.call(this, context);
	this.color = temp;
...
GridMorph.prototype.createLines = function () {
	...
	this.lines.forEach(function (line) {
		lm = new LineMorph(
			...
			myself.colors[i],
			this.isFilled
		); lm.setPosition(myself.position(
                 ).copy()); myself.add(lm); i += 1;
	});
};
...
LineMorph.prototype.rider = function () {
    ...
    rider.render = ChartMorph.prototype.renderGradient;
...
LineMorph.prototype.render = function (context) {
	// initialize my surface property
...
LineMorph.prototype.startStepping = function () {
	var myself = this; myself.rider().show();
...
) {this.step = null; this.rider().hide();};
...
ChartMorph.prototype.fixLayout = function () {
	...
	if (this.labelMorph) {
		...
		this.frame.setPosition(new Point(
			this.left() + inset,
		...
		this.zoomer.setPosition(this.frame.topRight().add(
			new Point(5, 0)
...
ChartMorph.prototype.zoom = function (w) {
	...
	offsetRatio = (this.frame.left() - this.contents.left()
	) / xunit; this.contents.setWidth(w);
...
ChartMorph.prototype.renderGradient = function (context) {
	var gradient;
	if ((this.edge === 0) && (this.border === 0)) {
		BoxMorph.uber.render.call(this, context);
		return;
