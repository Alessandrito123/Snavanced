...
const BigNumMemoizer = function () {this.init();}; BigNumMemoizer.prototype.saveOnTheStorage = function(selectedInput, selectedOutput) {if (!(isNil(this.localStorage))) {if (this.localStorage.getItem(
'bigNumMemoizerInputs') == '') {this.localStorage.setItem('bigNumMemoizerInputs', selectedInput);} else {this.localStorage.setItem('bigNumMemoizerInputs', (this.localStorage.getItem('bigNumMemoizerInputs'
)).concat('\|', selectedInput));}; if (this.localStorage.getItem('bigNumMemoizerOutputs') == '') {this.localStorage.setItem('bigNumMemoizerOutputs', selectedOutput);} else {this.localStorage.setItem(
'bigNumMemoizerOutputs', (this.localStorage.getItem('bigNumMemoizerOutputs')).concat('\|', selectedOutput));};};}; BigNumMemoizer.prototype.memoize = function (selectedFunction, selectedFirstInput,
selectedSecondInput) {var selectedString = selectedFunction.concat(',', selectedFirstInput.toString(), ',', isNil(selectedSecondInput) ? 'monadic' : (selectedSecondInput.toString()), ',', (BigDec
).prototype.maximumDecimals.toString()), selectedSearch = this.memoizedInputs.indexOf(selectedString); var result; if (selectedSearch > -1) {result = this.memoizedOutputs[selectedSearch];} else {
try {result = ((selectedFirstInput[selectedFunction])(selectedSecondInput)).toString();} catch (exception) {result = 'error'; console.error(exception);}; this.memoizedInputs.push(selectedString);
this.memoizedOutputs.push(result); if (false) {this.saveOnTheStorage(selectedString, result);};}; var selectedConstructor = (selectedFirstInput).constructor.name, receivedOutput = new Function(
'text', ('return \(new ').concat(selectedConstructor, '\(text\)\)\;')); if (result == 'error') {throw Error('Unsupported calculation.');} else {return receivedOutput(result);};}; (BigNumMemoizer
).prototype.clearTheCache = function () {if (!(isNil(this.localStorage))) {this.localStorage.setItem('bigNumMemoizerInputs', ''); this.localStorage.setItem('bigNumMemoizerOutputs', '');}; (this
).memoizedInputs = []; this.memoizedOutputs = [];}; BigNumMemoizer.prototype.init = function () {this.memoizedInputs = []; this.memoizedOutputs = []; this.localStorage = null; try {(this.localStorage
) = window.localStorage;} catch (error) {}; if (!isNil(this.localStorage)) {if (isNil(this.localStorage.getItem('bigNumMemoizerInputs'))) {this.localStorage.setItem('bigNumMemoizerInputs', '');}; if (
isNil(this.localStorage.getItem('bigNumMemoizerOutputs'))) {this.localStorage.setItem('bigNumMemoizerOutputs', '');}; this.memoizedInputs = (this.localStorage.getItem('bigNumMemoizerInputs')).split('\|'
...
BigDec.prototype.absoluteValue = function () {return (this.isNegative ? this.negativeCopy() : this.fullCopy());}; BigDec.prototype.simpleFactorial = function () {var factor = this.basicRound(); if ((factor
).isGreaterThan(theOne)) {return (factor.multiplicateWith(factor.addWith(theDarkUnit))).multiplicateWith(LocalBigNumMemoizer.memoize('simpleFactorial', factor.subtractWith(theFartNum)));} else {return theOne;
};}; BigDec.prototype.basicModulateWith = function (anotherBigNum) {if (this.isInfinite) {return theZero;} else if (anotherBigNum.isInfinite) {var copy = this.fullCopy(); if (anotherBigNum.isNegative) {(copy
).isNegative = !(copy.isNegative);}; return copy;} else {var firstNum = (this.absoluteValue()).toSignedInteger(), firstDot = this.dotPosition, secondNum = (anotherBigNum.absoluteValue()).toSignedInteger(),
secondDot = anotherBigNum.dotPosition, result; if (this.isEqualTo(anotherBigNum)) {return theZero;} else if (anotherBigNum.isEqualTo(theZero)) {return infinitySignedAs(this.isNegative);}; if (firstDot > (
secondDot)) {secondNum = secondNum.concat(getNZeroes(firstDot - secondDot)); result = BigDec(BigInt(firstNum) % BigInt(secondNum)); result.dotPosition = firstDot; result.isNegative = this.isNegative;
return result.fullCopy();} else if (secondDot > firstDot) {firstNum = firstNum.concat(getNZeroes(secondDot - firstDot)); result = BigDec(BigInt(firstNum) % BigInt(secondNum)); result.dotPosition = secondDot;
result.isNegative = this.isNegative; return result.fullCopy();} else {result = BigDec(BigInt(firstNum) % BigInt(secondNum)); result.dotPosition = firstDot; result.isNegative = this.isNegative; return (result
).fullCopy();};};}; BigDec.prototype.betterModulateWith = function (anotherBigNum) {if (this.isInfinite) {return theZero;} else if (anotherBigNum.isInfinite) {var copy = this.fullCopy(); if ((anotherBigNum
).isNegative) {copy.isNegative = !(copy.isNegative);}; return copy;} else if (anotherBigNum.isEqualTo(theZero)) {return infinitySignedAs(this.isNegative);} else {return ((this.basicModulateWith(anotherBigNum)
...
).fixDecimals();};}; BigDec.prototype.arcTangentRadians = function () {return summatoryIntegral((z => (theOne.addWith(z.squarePower()).getReciprocal())), this, theZero, BigDec(94));}; (BigDec.prototype
).min = function (anotherBigNum) {if (isNil(anotherBigNum)) {anotherBigNum = theZero;}; return [anotherBigNum, this][+(this.isLessThan(anotherBigNum))];}; BigDec.prototype.max = function (anotherBigNum
) {if (isNil(anotherBigNum)) {anotherBigNum = theZero;}; return [anotherBigNum, this][+(this.isGreaterThan(anotherBigNum))];}; BigDec.prototype.divideWith = function (divisor) {if (this.isInfinite || (
divisor.isInfinite)) {if (this.isInfinite && divisor.isInfinite) {return getUnitWithSign((this.isNegative + divisor.isNegative) == 1);} else {if (this.isInfinite) {return infinitySignedAs((((this.isNegative
) + divisor.isNegative) == 1));}; if (divisor.isInfinite) {return theZero;};};} else {var dividend = this.naturalContainer, divisorDotPosition = divisor.dotPosition, isNegative = ((this.isNegative + (divisor
).isNegative) == 1), quotient; divisor = divisor.naturalContainer; if (dividend == '0') {quotient = theZero;} else if ((dividend == divisor) && (this.dotPosition == divisorDotPosition)) {quotient = (
