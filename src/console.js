var localFonts = [], snavancedIsRunning = false; async function loadAFont (name, url) {var newFont = new FontFace(name, ('url\('
).concat(window.location.toString().split('snap.html')[0], url, '\)')); try {/* :~) */ await newFont.load(); document.fonts.add(
newFont); localFonts.push([url, name, newFont]);} catch (error) {console.error(error); localFonts.push([url, name, newFont]);}; if (
!(localFonts.length < 12) && !snavancedIsRunning) {initSnavanced();};}; function initSnavanced () {function detectPreferredLanguage () {
