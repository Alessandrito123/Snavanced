...
TimerFunction.prototype.getTime = function () {if (this.state === 'paused') {
var result = (this.pauseTime - this.resumeTime);} else {var result = (Date.now(
) - this.resumeTime);}; result = (isNaN(result) ? 0 : result); return result;};

TimerFunction.prototype.startNow = function () {this.stopNow(); this.resumeNow();};

TimerFunction.prototype.pauseNow = function () {if (this.state === 'running'
) {this.pauseTime = Date.now(); this.state = 'paused';};};

TimerFunction.prototype.resumeNow = function () {if (this.state === 'paused') {this.resumeTime = (
(Date.now() - this.pauseTime) + this.resumeTime); this.state = 'running';};};

TimerFunction.prototype.stopNow = function () {this.startTime = Date.now();
this.pauseTime = this.startTime; this.resumeTime = this.pauseTime; this.state = 'paused';};

TimerFunction.prototype.toString = function () {return ('timer function \(\) \{return ').concat((this.getTime() / 1000), '\;\}');};

TimerFunction.prototype.toCronoString = function (optionalNumber) {if (isNil(optionalNumber)) {var currentTime = this.getTime() / 1000;} else {
...
Math.trunc((currentTime * 1000) % 1000).toString(), 'MS' );}; /* Optional, but the numbers are now with text to be more complex like a real timer. */

function isSnapObject (thing) {return asABool((thing instanceof SpriteMorph) || (thing instanceof StageMorph));};
...
3); SpriteMorph.prototype.bubbleBorderColor = new Color(190, 190, 190); SpriteMorph.prototype.bubbleColor = WHITE; SpriteMorph.prototype.bubbleMaxTextWidth = 130; SpriteMorph.prototype.initBlocks = function () {
    ...
    SpriteMorph.prototype.blocks = {
        ...
        forward: {
            ...
            type: 'command',
            category: 'motion',
            spec: 'SELECTOR forward INPUTS %n %optDir',
            defaults: [10]
        },
        bounceOffEdge: {
            only: SpriteMorph,
            type: 'command',
            category: 'motion',
            spec: 'SELECTOR bounceOffEdge'
        },
        turn: {
            only: SpriteMorph,
            type: 'command',
            category: 'motion',
            spec: 'SELECTOR turn INPUTS %n degrees',
            defaults: [15]
        },
        turnLeft: {
            only: SpriteMorph,
            type: 'command',
            category: 'motion',
            spec: 'SELECTOR turnLeft INPUTS %n degrees',
            defaults: [15]
        },
        setHeading: {
            only: SpriteMorph,
            type: 'command',
            category: 'motion',
            spec: 'SELECTOR setHeading INPUTS %dir',
            defaults: [90]
        },
        doFaceTowards: {
            only: SpriteMorph,
        ...
        doScrollStage: {
            ...
            type: 'command',
            category: 'motion',
            spec: '%setAndChange the scroll %xAndY to/by %n',
            defaults: ['change', 'x', 10]
        }, */
        ...
        motorDirection: {
            type: 'command',
            category: 'motor',
            spec: 'motor direction %motorMenu',
            defaults: ['this way']
        },
        ...
        reportNewCostumeSkewed: {
            type: 'reporter',
            category: 'looks',
            spec: 'skew %cst to %dir degrees in %n %',
        ...
        customizableBubble: {
            ...
            type: 'command',
            category: 'looks',
            spec: '%bubbleOptions %mlt %timingOptions',
            defaults: [['say'], localize('Hello!')]
        },
        getBubbleContents: {
            only: SpriteMorph,
            type: 'reporter',
        ...
        makeMIDSound: {
            type: 'reporter',
            category: 'sound',
            spec: 'MID with: %br note: %note %br volume: %n %br duration: %n %br instrument: %inst %br %optNum',
            defaults: [60, 100, 44100, 1]
        },
        makeNoiseSound: {
            type: 'reporter',
            category: 'sound',
        ...
        fillOval: {
            ...
            type: 'command',
            category: 'pen',
            spec: 'oval with\: %br width\: %n %br height\: %n',
            defaults: [60, 60]
        },
        fillRectangle: {
            only: SpriteMorph,
            type: 'command',
            category: 'pen',
            spec: 'rectangle with\: %br width\: %n %br height\: %n %br roundness\: %n',
            defaults: [60, 60, 0]
        },
        ...
        mixClrsAt: {
            type: 'reporter',
            category: 'pen',
            spec: 'mix %clr with %clr at %n %',
            defaults: [null, null, 50]
        },
        colorAttr: {
            type: 'reporter',
            category: 'pen',
            spec: '%colorAttrs of %clr',
            defaults: [localize('hex')]
        },
        clrFlags: {
            type: 'reporter',
            category: 'pen',
            spec: '%clrFlags %clr in %n %',
            defaults: [localize('light up'), null, 10]
        },
        ...
        runScript: {
            type: 'definitor',
            category: 'events',
            spec: 'when %flagTheme clicked %c',
            dev: true
        },
        receiveGo: {
            type: 'hat',
            category: 'events',
            spec: 'when %flagTheme clicked'
        },
        receiveKey: {
            type: 'hat',
            category: 'events',
        ...
        doBroadcast: {
            type: 'command',
            category: 'events',
            spec: 'do broadcast %msg %receive'
        },
        doBroadcastAndWait: {
            type: 'command',
            category: 'events',
            spec: 'do broadcast %msg %receive and wait'
        },
        stopTheMessage: {
            type: 'command',
            category: 'events',
            spec: 'stop broadcast %msg'
        },
        ...
        doRepeat: {
            type: 'command',
            category: 'control',
            spec: 'repeat %n times %loop',
            defaults: [10]
        },
        doTimrLp: {
            type: 'command',
            category: 'control',
            spec: 'repeat %n secs %loop',
            defaults: [1]
        },
        doUntil: {
            type: 'command',
        ...
        doStopThis: {
            type: 'command',
            category: 'control',
            spec: 'stop %stopChoices $octagon-1.5-200-0-0',
            defaults: [['this scene']]
        },
        stopTheScript: {
            type: 'command',
            category: 'control',
            spec: 'stop script %scripts'
        },
        ...
        reportPipe: {
            type: 'reporter',
            category: 'control',
            spec: 'pipe %s $arrowRight %mult%r'
        },
        reportSequence: {
            type: 'reporter',
            category: 'control',
            spec: 'sequence %r $arrowRight %mult%s'
        },
        doReport: {
            type: 'command',
            category: 'control',
        ...
        doCallCC: {
            type: 'command',
            category: 'control',
            spec: 'run %cmd w/continuation'
        },
        reportCallCC: {
            type: 'reporter',
            category: 'control',
            spec: 'call %r w/continuation'
        },
        doCatch: {
            type: 'command',
            category: 'control',
            spec: 'catch %upvar %c',
            defaults: [localize('tag')]
        },
        throw: {
            type: 'command',
            category: 'control',
            spec: 'throw %s',
            defaults: [localize('catchtag')]
        },
        doWarp: {
            type: 'command',
            category: 'control',
        ...
        doTellTo: {
            type: 'command',
            category: 'control',
            spec: 'tell %spr to %cmd %inputs'
        },
        tellToCT: {
            dev: true,
            type: 'command',
            category: 'control',
            spec: 'tell %spr to %cs %inputs'
        },
        reportAskFor: {
            type: 'reporter',
            category: 'control',
            spec: 'ask %spr for %r %inputs'
        },
        ...
        tryCatch: {
            type: 'command',
            category: 'control',
            defaults: [null, null, localize('error')],
            spec: 'safely try %cl then run %cl and throw %upvar'
        },
        launchError: {
            type: 'command',
            spec: 'error %mlt',
            category: 'control',
            defaults: [localize('Show your\nerror here.')]
        },
        ...
        doSetBlockAttribute: {
            type: 'command',
            category: 'control',
            spec: 'set %s of block %repRing to %s',
            defaults: ['label']
        },
        doDeleteBlock: {
            type: 'command',
        ...
        doPauseOptions: {
            type: 'command',
            category: 'control',
            spec: 'pause %pauseOptions $pause-1.5-255-220-0',
            defaults: [['this scene']]
        },
        doResumeOptions: {
            type: 'command',
            category: 'control',
            spec: 'unpause %resumeOptions $pointRight-1.5-255-220-0',
            defaults: [['this scene']]
        },
        doPauseThread: {
            type: 'command',
        ...
        scriptNamer: {
            type: 'command',
            category: 'control',
            spec: 'script %commentTxt %scriptHidden',
            defaults: [localize('A blank script.')]
        },
        staticComment: {
            type: 'command',
            category: 'other',
            spec: 'comment %commentTxt',
            defaults: ['Please edit\nthis comment.']
        },
        blockComment: {
            type: 'command',
            category: 'other',
            spec: 'comment %commentHidden',
            defaults: [localize('Please edit\nthis comment.')]
        },
        scriptChanger: {
            type: 'ring',
            category: 'other',
            spec: '%scriptChanger %r %selector',
            defaults: [['commandize']]
        },
        ...
        doAsk: {
            ...
            category: 'sensing',
            spec: 'ask %s and wait',
            defaults: [localize('What is your own name?')]
        },
        getLastAnswer: {
            type: 'reporter',
            category: 'sensing',
        ...
        reportMouseButton: {
            type: 'predicate',
            category: 'sensing',
            spec: '%mouseButtons mouse button down?',
            defaults: [['left']]
        },
        reportKeyPressed: {
            type: 'predicate',
        ...
        doChangeTimer: {
            type: 'command',
            category: 'sensing',
            spec: '%timerOptions timer',
            defaults: [['reset']]
        },
        doSetTimer: {
            type: 'command',
            category: 'sensing',
            spec: '%setAndChange timer %br in %n seconds',
            defaults: [['change'], 5]
        },
        isTimerRunning: {
            type: 'predicate',
        ...
        getBatteryChargingTime: {
            type: 'reporter',
            category: 'sensing',
            spec: 'battery\'s %batteryMenu time',
            defaults: [['charging']]
        },
        getBatteryIsCharging: {
            type: 'predicate',
            category: 'sensing',
            spec: 'the battery is charging?'
        },
        reportAttributeOf: {
            type: 'reporter',
            category: 'sensing',
        ...
        doURL: {
            ...
            category: 'sensing',
            spec: 'open url %s',
            defaults: ['https\:\/\/snap.berkeley.edu']
        },
        reportURL: {
            type: 'reporter',
            category: 'sensing',
        ...
        createCategory: {
            type: 'command',
            category: 'sensing',
            spec: 'create category %br with name %s %br and color %clr',
            defaults: ['Test']
        },
        renameCategory: {
            type: 'command',
            category: 'sensing',
            spec: 'rename the category %br %s to %s',
            defaults: ['Test', 'Test2']
        },
        setCategoryColor: {
            type: 'command',
            category: 'sensing',
            spec: 'set the color of the %br category %s to %clr',
            defaults: ['Test']
        },
        deleteCategory: {
            type: 'command',
            category: 'sensing',
            spec: 'delete the category %s',
            defaults: ['Test']
        },
        ...
        reportConstrainToClamp: {
            type: 'reporter',
            category: 'operators',
            spec: 'constrain %n from %n to %n with clamping from %n to %n'
        },
        reportInterpolate: {
            type: 'reporter',
            category: 'operators',
            spec: 'interpolate %n to %n by %n'
        },
        reportAverage: {
            type: 'reporter',
            category: 'operators',
            spec: '%averageOptions of %averageNumbers',
            defaults: [['median']]
        },
        ...
        reportNumeralBase: {
            type: 'reporter',
            category: 'operators',
            spec: 'get the base of %n',
            defaults: [255]
        },
        reportNumeralText: {
            type: 'reporter',
            category: 'operators',
            spec: 'get the text of %n',
            defaults: [255]
        },
        decomposeAPolynomicallyNumeral: {
            type: 'reporter',
            category: 'operators',
            spec: '%s from base %n',
            defaults: ['FF', 16]
        },
        reportNumberWithMoreDigits: {
            type: 'reporter',
            category: 'operators',
            spec: '%s with %n digits',
            defaults: ['255', 3]
        },
        ...
        reportTextContains: {
            ...
            category: 'operators',
            spec: '%s contains %s ?',
            defaults: [localize('hello') + ' ' + localize('world'), localize('world')]
        },
        reportUnicode: {
            type: 'reporter',
            category: 'operators',
        ...
        */
        ...
        reportListIndexFixed: {
            type: 'reporter',
            category: 'lists',
            spec: '%indexing of %s in %l',
            defaults: [['index'], localize('thing')]
        },
        doAddToList: {
            type: 'command',
            category: 'lists',
        ...
        reportExpandedList: {
            type: 'reporter',
            category: 'lists',
            spec: 'add %n empty items to %l',
            defaults: [10, null]
        },
        doSetListLength: {
            type: 'command',
            category: 'lists',
            spec: 'set %l \'s length to %n items',
            defaults: [null, 10]
        },
        reportListAtThatLength: {
            type: 'reporter',
            category: 'lists',
            spec: 'copy %l with the length of %n items',
            defaults: [null, 10]
        },
        reportReshape: {
            type: 'reporter',
        ...
        reportFindFirstFixed: {
            type: 'reporter',
            category: 'lists',
            spec: 'find first %findfirst %p in %l',
            defaults: [['item']]
        },
        reportAtomicFindFirstFixed: {
            type: 'reporter',
            category: 'lists',
            spec: '$flash find first %findfirst %p in %l',
            defaults: [['item']]
        },
        reportCombine: {
            type: 'reporter',
        ...
        callMeWith: {
            type: 'reporter',
            category: 'random',
            spec: 'Wow, you are a smart one. Now call ME %s !'
        },
        runMeWith: {
            type: 'command',
            category: 'random',
...
SpriteMorph.prototype.initBlockMigrations = function () {
    SpriteMorph.prototype.blockMigrations = {
        ...
        */
        ...
        reportDeepListCopy: {
            selector: 'reportListCopy'
        },
        reportMouseDown: {
            selector: 'reportMouseButton',
            inputs: [['left']]
        },
        doResetTimer: {
            selector: 'doChangeTimer',
            inputs: [['reset']]
        },
        reportTimer: {
            selector: 'getTimer'
        ...
        reportListIndex: {
            selector: 'reportListIndexFixed',
            inputs: [['index']],
            offset: 1
        },
        reportFindFirst: {
            selector: 'reportFindFirstFixed',
            inputs: [['item']],
            offset: 1
        },
        reportAtomicFindFirst: {
            selector: 'reportAtomicFindFirstFixed',
            inputs: [['item']],
            offset: 1
        },
        doSend: {
...
SpriteMorph.prototype.blockAlternatives = {
    ...
    // control:
    doBroadcast: ['doBroadcastAndWait'], doBroadcastAndWait: ['doBroadcast'],
    doWaitUntil: ['doWaitWhile'], doWaitWhile: ['doWaitUntil'], doRepeat: [
    'doTimrLp', 'doUntil', ['doForever', -1], ['doFor', 2], ['doForEach',
    1]], doTimrLp: ['doRepeat', 'doUntil', ['doForever', -1], ['doFor', 2],
    ['doForEach', 1]], doUntil: ['doWhile'], doWhile: ['doUntil'], doRun: [
    'fork', 'doCallCC'], fork: ['doRun', 'doCallCC'], doCallCC: ['doRun',
    'fork'], evaluate: ['reportCallCC', 'reportSequence'], reportCallCC: [
    'evaluate', 'reportSequence'], reportSequence: ['evaluate', 'reportCallCC'
    ], doTellTo: ['tellToCT'], tellToCT: ['doTellTo'], doPauseThread: [
    'doPauseScript'], doPauseScript: ['doPauseThread'],
    ...
    // operators:
    reportVariadicSum: ['reportDifference', 'reportVariadicProduct', 'reportQuotient', 'reportModulus', 'reportPower',
    'reportRadical', 'reportLogarithm', 'reportRandom', 'reportAtan2', 'reportVariadicMin', 'reportVariadicMax'],
    reportDifference: ['reportVariadicSum', 'reportVariadicProduct', 'reportQuotient', 'reportModulus', 'reportPower',
    'reportRadical', 'reportLogarithm', 'reportRandom', 'reportAtan2', 'reportVariadicMin', 'reportVariadicMax'],
    reportVariadicProduct: ['reportVariadicSum', 'reportDifference', 'reportQuotient', 'reportModulus', 'reportPower',
    'reportRadical', 'reportLogarithm', 'reportRandom', 'reportAtan2', 'reportVariadicMin', 'reportVariadicMax'],
    reportQuotient: ['reportVariadicSum', 'reportDifference', 'reportVariadicProduct', 'reportModulus', 'reportPower',
    'reportRadical', 'reportLogarithm', 'reportRandom', 'reportAtan2', 'reportVariadicMin', 'reportVariadicMax'],
    reportModulus: ['reportVariadicSum', 'reportDifference', 'reportVariadicProduct', 'reportQuotient', 'reportPower',
    'reportRadical', 'reportLogarithm', 'reportRandom', 'reportAtan2', 'reportVariadicMin', 'reportVariadicMax'],
    reportPower: ['reportVariadicSum', 'reportDifference', 'reportVariadicProduct', 'reportQuotient', 'reportModulus',
    'reportRadical', 'reportLogarithm', 'reportRandom', 'reportAtan2', 'reportVariadicMin', 'reportVariadicMax'],
    reportRadical: ['reportVariadicSum', 'reportDifference', 'reportVariadicProduct', 'reportQuotient', 'reportModulus',
    'reportPower', 'reportLogarithm', 'reportRandom', 'reportAtan2', 'reportVariadicMin', 'reportVariadicMax'],
    reportLogarithm: ['reportVariadicSum', 'reportDifference', 'reportVariadicProduct', 'reportQuotient', 'reportModulus',
    'reportPower', 'reportRadical', 'reportRandom', 'reportAtan2', 'reportVariadicMin', 'reportVariadicMax'],
    reportRandom: ['reportVariadicSum', 'reportDifference', 'reportVariadicProduct', 'reportQuotient', 'reportModulus',
    'reportPower', 'reportRadical', 'reportLogarithm', 'reportAtan2', 'reportVariadicMin', 'reportVariadicMax'],
    reportAtan2: ['reportVariadicSum', 'reportDifference', 'reportVariadicProduct', 'reportQuotient', 'reportModulus',
    'reportPower', 'reportRadical', 'reportLogarithm', 'reportRandom', 'reportVariadicMin', 'reportVariadicMax'],
    reportVariadicMin: ['reportVariadicSum', 'reportDifference', 'reportVariadicProduct', 'reportQuotient', 'reportModulus',
    'reportPower', 'reportRadical', 'reportLogarithm', 'reportRandom', 'reportAtan2', 'reportVariadicMax'],
    reportVariadicMax: ['reportVariadicSum', 'reportDifference', 'reportVariadicProduct', 'reportQuotient', 'reportModulus',
    'reportPower', 'reportRadical', 'reportLogarithm', 'reportRandom', 'reportAtan2', 'reportVariadicMin'],

    reportLessThan: ['reportLessThanOrEquals', 'reportVariadicEquals',
        'reportVariadicIsIdentical', 'reportVariadicNotEquals', 'reportGreaterThan',
        'reportGreaterThanOrEquals'],
    reportVariadicEquals: ['reportVariadicIsIdentical', 'reportVariadicNotEquals', 'reportLessThan',
        'reportLessThanOrEquals', 'reportGreaterThan',
        'reportGreaterThanOrEquals'],
    reportVariadicNotEquals: ['reportVariadicEquals', 'reportVariadicIsIdentical', 'reportLessThan',
        'reportLessThanOrEquals', 'reportGreaterThan',
        'reportGreaterThanOrEquals'],
    reportGreaterThan: ['reportGreaterThanOrEquals', 'reportVariadicEquals',
        'reportVariadicIsIdentical', 'reportVariadicNotEquals', 'reportLessThan',
        'reportLessThanOrEquals'],
    reportLessThanOrEquals: ['reportLessThan', 'reportVariadicEquals',
        'reportVariadicIsIdentical', 'reportVariadicNotEquals', 'reportGreaterThan',
        'reportGreaterThanOrEquals'],
    reportGreaterThanOrEquals: ['reportGreaterThan', 'reportVariadicEquals',
        'reportVariadicIsIdentical', 'reportVariadicNotEquals', 'reportLessThan',
        'reportLessThanOrEquals'],
    reportVariadicIsIdentical: ['reportVariadicEquals', 'reportVariadicNotEquals', 'reportLessThan',
        'reportLessThanOrEquals', 'reportGreaterThan',
        'reportGreaterThanOrEquals'],
    reportVariadicAnd: ['reportVariadicOr', 'reportXor'],
    reportVariadicOr: ['reportVariadicAnd', 'reportXor'],
    reportXor: ['reportVariadicAnd', 'reportVariadicOr'],
    ...
    // HOFs
    reportMap: ['reportKeep', ['reportFindFirstFixed', 1]],
    reportKeep: [['reportFindFirstFixed', 1], 'reportMap'],
    reportFindFirstFixed: [['reportKeep', -1], ['reportMap', -1]],
    reportAtomicMap: ['reportAtomicKeep', ['reportAtomicFindFirstFixed', 1]],
    reportAtomicKeep: [['reportAtomicFindFirstFixed', 1], 'reportAtomicMap'],
    reportAtomicFindFirstFixed: [['reportAtomicKeep', -1], ['reportAtomicMap', -1]],
    doForEach: [['doFor', 1], ['doForever', -2], ['doRepeat',
    -1], ['doTimrLp', -1], ['doUntil', -1], ['doWhile', -1]]
};
...
SpriteMorph.prototype.fullCopy = function (forClone) {
    ...
    if (forClone) {
        ...
        if ((c.costume instanceof Costume
        ) && !(c.getCostumeIdx())) {
...
SpriteMorph.prototype.fixLayout = function () {
    ...
    if (this.rotationStyle === 2) {facing = 90;
        this.heading = ((this.heading < 0) ? (
        360 - (Math.abs(this.heading) % 360)) : (
        this.heading % 360)); isFlipped = !(
        this.heading < 180);  /* Very good! */
        ...
        if (this.rotationStyle === 1) { // rotate freely in all directions
            ...
            this.imageExtent = new Point(
            imageSide, imageSide);} else {
        ...
        newX = (this.normalExtent.x * this.scale * stageScale);
        this.bounds.setWidth(newX); this.bounds.setHeight(newX);
...
SpriteMorph.prototype.render = function (ctx) {
    ...
    if (this.rotationStyle === 2) {facing = 90;
        ...
        pic = isFlipped ? this.costume.flipped() : this.costume; ctx.save();
        ctx.scale(this.scale * stageScale, this.scale * stageScale);
        ctx.translate(Math.round((this.imageOffset).x),
        Math.round((this.imageOffset).y));
    ...
    this.cachedImage = this.applyGraphicsEffects(
...
SpriteMorph.prototype.getImageData = function () {
    ...
    // Get sprite image data scaled to 1 an converted ABGR array,
...
SpriteMorph.prototype.projectionSnap = function() {
    ...
    ); snap = newCanvas(new Point(w, h
    ), true); ctx = snap.getContext(
    '2d'); ctx.drawImage(cst, 0, 0);
...
SpriteMorph.prototype.blockForSelector = function (selector, setDefaults) {
    ...
    : info.type === 'definitor' ? new DefinitorBlockMorph
        : info.type === 'hat' ? new HatBlockMorph
            : info.type === 'ring' ? new RingMorph
                : new ReporterBlockMorph((info.type === 'predicate'), (info.type === 'arrow'));
    block.color = (info.category === 'random' ? (new Color((Math.random() * 255), (Math.random(
    ) * 255), (Math.random() * 255))) : this.blockColorFor(info.category));
    block.category = info.category; block.selector = (
    migration ? migration.selector : selector);
    block.setSpec(localize(info.spec)); if ((
    migration && migration.expand)) {if (migration.expand instanceof Array) {
...
var blocks = [], myself = this, varNames, inheritedVars = this.inheritedVariableNames(), devMode = world.isDevMode; /* We are sorted correctly. */
    ...
    function block(selector, isGhosted) {
        if (StageMorph.prototype.hiddenPrimitives[selector] && !all) {
            return null;
        };  var newBlock = SpriteMorph.prototype.blockForSelector(
        selector, true); newBlock.isTemplate = true;
        ...
        if (contains(inheritedVars, varName)) {newBlock.ghost();};
        return newBlock;
    }; function watcherToggle(selector) {
        if (StageMorph.prototype.hiddenPrimitives[selector]) {return null;};
        var info = SpriteMorph.prototype.blocks[selector],
        query = function () {return myself.showingWatcher(
        selector);}, toggleButton = new ToggleMorph(
            'checkbox',
            null,
            function () {myself.toggleWatcher(selector, localize(info.spec), myself.blockColor[info.category]);
            this.hint = (query() ? 'Remove viewer from stage' : 'View on stage');},
            null,
            query,
            null,
        ...
        var query = function () {return myself.showingVariableWatcher(
        varName);}, toggleButton = new ToggleMorph(
            'checkbox',
            null,
            function () {myself.toggleVariableWatcher(varName);
            this.hint = (query() ? 'Remove viewer from stage' : 'View on stage');},
            null,
            query,
            null,
        ...
        } else {blocks.push(this.subCategoryText('Motion'));
        blocks.push('-');
        ...
        blocks.push('-');
        blocks.push(this.subCategoryText('Tweening'));
        blocks.push('-');
        blocks.push(block('doGlide'));
        blocks.push(block('doMove'));
        blocks.push(block('doShake'));
        blocks.push('-');
        blocks.push(this.subCategoryText('Position'));
        blocks.push('-');
        ...
        }; if (IDE_Morph.showMotorBlocks) {
        blocks.push('-');
        blocks.push(this.subCategoryText('Motors'));
        blocks.push('-');
        blocks.push(block('motorOnAndWait'));
        blocks.push(block('motorOn'));
        blocks.push(block('motorOff'));
        blocks.push(block('motorPower'));
        blocks.push(block('motorDirection'));
        };

    } else if (category === 'looks') {

        blocks.push(this.subCategoryText((this instanceof StageMorph
        ) ? 'Backgrounds' : 'Costumes')); blocks.push('-');
        if (this instanceof StageMorph) {
        blocks.push(block('doSwitchToBackground'));
        blocks.push(block('doChangeToNextBackground'));
        blocks.push(watcherToggle('getBackgroundIdx'));
        ...
        blocks.push(block('doEditCostume'));
        ...
        if (this instanceof SpriteMorph) {blocks.push('-'
        ); blocks.push(this.subCategoryText('Talking'));
        blocks.push('-'); blocks.push(block(
        'customizableBubble')); blocks.push(
        block('getBubbleContents'));}; blocks.push('-');
        blocks.push(this.subCategoryText('Effects'));
        blocks.push('-');
        ...
        blocks.push('-');
        blocks.push(this.subCategoryText('Size'));
        blocks.push('-');
        ...
        blocks.push('-');
        blocks.push(this.subCategoryText('Viewing'));
        blocks.push('-');
        ...
        blocks.push('-');
        blocks.push(this.subCategoryText('Layers'));
        blocks.push('-');
        ...
        blocks.push(this.subCategoryText('Waveforms'));
        blocks.push('-');
        blocks.push(block('makeMIDSound'));
        blocks.push(block('makeNoiseSound'));
        blocks.push('-');
        blocks.push(this.subCategoryText('Sounds'));
        blocks.push('-');
        blocks.push(block('playBeep'));
        blocks.push('-');
        ...
        blocks.push(block('getBackgroundAttribute'));
        } else {blocks.push(block('downOrUp'));
        ...
        blocks.push(block('doErase'));
        blocks.push(block('floodFill'));
        blocks.push(block('write'));
        blocks.push('-');
        blocks.push(block('fillOval'));
        blocks.push(block('fillRectangle'));};
        blocks.push('-');
        ...
        blocks.push('-');
        blocks.push(this.subCategoryText('Colors'));
        blocks.push('-');
        blocks.push(block('makeColor'));
        blocks.push(block('getColor'));
        blocks.push(block('colorAttr'));
        blocks.push(block('clrFlags'));
        blocks.push(block('mixColors'));
        blocks.push(block('mixClrsAt'));

    } else if (category === 'control') {

        blocks.push(this.subCategoryText('Triggers'));
        blocks.push('-');
        ...
        blocks.push('-');
        blocks.push(this.subCategoryText('Messages'));
        blocks.push('-');
        ...
        blocks.push('-');
        blocks.push(block('doCatch'));
        blocks.push(block('throw'));
        blocks.push('-');
        blocks.push(this.subCategoryText('Looping'));
        blocks.push('-');
        ...
        blocks.push('-');
        blocks.push(this.subCategoryText('Debugging'));
        blocks.push('-');
        blocks.push(block('doWarp'));
        blocks.push('-');
        blocks.push(block('doPauseOptions'));
        blocks.push(block('doResumeOptions'));
        blocks.push('-');
        ...
        blocks.push(block('launchError'));
        blocks.push('-');
        blocks.push(this.subCategoryText('Stopping'));
        blocks.push('-');
        blocks.push(block('doReport'));
        blocks.push(block('doStopThis'));
        blocks.push(block('stopTheScript'));
        blocks.push('-');
        blocks.push(this.subCategoryText('Introspecting'));
        blocks.push('-');
        blocks.push(block('reportBlockAttribute'));
        blocks.push('-');
        blocks.push(this.subCategoryText('Scripting'));
        blocks.push('-');
        blocks.push(block('execute'));
        blocks.push(block('evaluate'));
        blocks.push(block('doTellTo'));
        blocks.push(block('reportAskFor'));
        blocks.push(block('reportPipe'));
        blocks.push(block('reportSequence'));
        ...
        blocks.push(watcherToggle('reportSceneName'));
        blocks.push(block('reportSceneName'));
        blocks.push(watcherToggle('reportSceneNumber'));
        blocks.push(block('reportSceneNumber'));
        blocks.push('-');
        blocks.push(this.subCategoryText('Extras'));
        blocks.push('-');
        blocks.push(block('doIgnoreValue'));
        blocks.push(block('scriptNamer'));
        blocks.push(block('blockComment'));
        blocks.push(block('scriptChanger'));
        blocks.push('#');
        ...
        blocks.push(block('reportMouseButton'));
        blocks.push('-');
        ...
        blocks.push('-');
        blocks.push(block('doChangeTimer'));
        blocks.push(block('doSetTimer'));
        blocks.push(watcherToggle('isTimerRunning'));
        blocks.push(block('isTimerRunning'));
        blocks.push(watcherToggle('getTimer'));
        blocks.push(block('getTimer'));
        blocks.push('-');
        blocks.push(this.subCategoryText(localize('Battery')));
        blocks.push('-');
        blocks.push(watcherToggle('getBatteryLevel'));
        blocks.push(block('getBatteryLevel'));
        blocks.push(block('getBatteryChargingTime'));
        blocks.push(watcherToggle('getBatteryIsCharging'));
        blocks.push(block('getBatteryIsCharging'));
        blocks.push('-');
        ...
        blocks.push('-');
        blocks.push(block('doURL'));
        blocks.push(block('reportURL'));
        blocks.push('-');
        blocks.push(block('reportAudio'));
        blocks.push(watcherToggle('reportLoud'));
        blocks.push(block('reportLoud'));
        blocks.push('-');
        ...
        if (!asABool(localStorage['-snap-setting-oldLambdaOn'])) {
        blocks.push(this.subCategoryText(localize('Lambda')));
        blocks.push('-'); blocks.push(block('reifyScript'));
        blocks.push(block('reifyReporter')); blocks.push(
        block('reifyPredicate')); blocks.push('#'); (blocks
        ).push('-');}; blocks.push((this.subCategoryText)(
        'Operators')); blocks.push('-');
        ...
        blocks.push(block('reportRadical'));
        blocks.push(block('reportLogarithm'));
        blocks.push(block('reportRandom'));
        blocks.push(block('reportIntegral'));
        blocks.push(block('reportAtan2'));
        blocks.push(block('reportVariadicMin'));
        blocks.push(block('reportVariadicMax'));
        blocks.push('-');
        blocks.push(this.subCategoryText('Values'));
        blocks.push('-');
        blocks.push(block('reportConstrainToClamp'));
        blocks.push(block('reportInterpolate'));
        blocks.push(block('reportAverage'));
        blocks.push(block('reportNumbers'));
        blocks.push(block('reportCombsNPerms'));
        blocks.push(block('reportMonadic'));
        blocks.push(block('reportComplexNumAttrs'));
        blocks.push(block('reportRound'));
        blocks.push(block('reportConstants'));
        blocks.push('-');
        blocks.push(this.subCategoryText('Bases'));
        blocks.push('-');
        blocks.push(block('reportNewNumeral'));
        blocks.push(block('reportNumeralBase'));
        blocks.push(block('reportNumeralText'));
        blocks.push(block('decomposeAPolynomicallyNumeral'));
        blocks.push(block('reportNumberWithMoreDigits'));
        blocks.push('-');
        blocks.push(this.subCategoryText('Comparatives'));
        blocks.push('-');
        ...
        blocks.push(block('reportVariadicNotEquals'));
        blocks.push(block('reportGreaterThanOrEquals'));
        blocks.push(block('reportGreaterThan'));
        blocks.push('-');
        blocks.push(this.subCategoryText('Logics'));
        blocks.push('-');
        blocks.push(block('reportVariadicAnd'));
        blocks.push(block('reportVariadicOr'));
        blocks.push(block('reportXor'));
        blocks.push(block('reportNot'));
        blocks.push(block('reportBoolean'));
        blocks.push('-');
        blocks.push(this.subCategoryText('Strings'));
        blocks.push('-');
        ...
        blocks.push(block('reportTextContains'));
        blocks.push('-');
        blocks.push(this.subCategoryText('Unicode'));
        blocks.push('-');
        blocks.push(block('reportUnicode'));
        blocks.push(block('reportUnicodeAsLetter'));
        blocks.push('-');
        blocks.push(this.subCategoryText('Types'));
        blocks.push('-');
        ...
        blocks.push(this.subCategoryText(localize('Lambda')));
        blocks.push('-');
        blocks.push(block('reportScript'));
        blocks.push('#');
        blocks.push(block('reify'));
        blocks.push('#');
        }; blocks.push('-');
        if (this instanceof SpriteMorph) {blocks.push('-');
        blocks.push(this.subCategoryText(localize('Cloning')
        )); blocks.push('-'); blocks.push(block('spawn'));};

        if (Process.prototype.enableJS) {
            blocks.push('-');
            blocks.push(this.subCategoryText(localize('JavaScript')));
            blocks.push('-');
        ...
        if (varNames.length > 0) {
            blocks.push(this.subCategoryText(localize('Global Variables')));
            blocks.push('-');
        ...
        if (varNames.length > 0) {
            blocks.push(this.subCategoryText(localize('Local Variables')));
            blocks.push('-');
            varNames.forEach(name => {
                blocks.push(variableWatcherToggle(name));
        ...
        blocks.push(block('doShowingVarOptions'));
        ...
        if (StageMorph.prototype.enableInheritance) {
            blocks.push('-');
            blocks.push(this.subCategoryText(localize('Inheritance')));
            blocks.push('-');
        ...
        blocks.push(block('reportListIndexFixed'));
        blocks.push(block('reportListContainsItem'));
        blocks.push(block('reportListOnlyHasItem'));
        blocks.push(block('reportListIsEmpty'));
        blocks.push(block('reportListContents'));
        blocks.push('-');
        blocks.push(block('reportListCopy'));
        blocks.push('-');
        blocks.push(block('reportMap'));
        blocks.push(block('reportKeep'));
        blocks.push(block('reportFindFirstFixed'));
        ...
        blocks.push(block('reportExpandedList'));
        blocks.push(block('doDeleteFromList'));
        blocks.push(block('doInsertInList'));
        blocks.push(block('doReplaceInList'));
        blocks.push(block('reportListCombination'));
        blocks.push('-');
        blocks.push(block('doSetListLength'));
        blocks.push(block('reportListAtThatLength'));
        blocks.push('-');
        ...
        if (IDE_Morph.prototype.whiteExtensionsList.length === 0) {
        blocks.push(this.noExtensionsText());
        };
        ...
        if (contains(IDE_Morph.prototype.whiteExtensionsList, 'Video Sensing')) {
        blocks.push('-');
        blocks.push(this.subCategoryText(localize('Video Sensing Extension')));
        blocks.push('-');
        blocks.push(block('doSetVideoTransparency'));
        blocks.push(block('doSetVideoToMode'));
        blocks.push(block('reportVideo'));
        };

        if (contains(IDE_Morph.prototype.whiteExtensionsList, 'Text To Speech')) {
        blocks.push('-');
        blocks.push(this.subCategoryText(localize('Text To Speech Extension')));
        blocks.push('-');
        blocks.push(block('doSpeakAndWait'));
        blocks.push(block('doSetSpeakingPitch'));
        blocks.push(block('doSetSpeakingSpeed'));
        blocks.push(block('doSetSpeakingLanguage'));
        };
    };
...
SpriteMorph.prototype.devModeText = function () {var txt = new TextMorph(localize('development mode\ndebugging primitives:')); txt.fontSize = 9; txt.setColor(this.paletteTextColor); return txt;};
SpriteMorph.prototype.noExtensionsText = function () {var txt = new TextMorph(localize('There are no extensions added by now.')); txt.fontSize = 9; txt.setColor(this.paletteTextColor); return txt;};

SpriteMorph.prototype.helpMenu = function () {var menu = new MenuMorph(this); menu.addItem('help...', 'showHelp'); return menu;};

SpriteMorph.prototype.customBlockTemplatesForCategory = function (category, includeHidden
) {if (contains(Array.from(SpriteMorph.prototype.customCategories.keys()), category)) {
var ide = this.parentThatIsA(IDE_Morph), blocks = [], isInherited = false, block, inheritedBlocks;
function addCustomBlock(definition) {if ((!definition.isHelper || includeHidden) && definition.category === category) {
block = definition.templateInstance(); if (isInherited) {block.ghost();}; blocks.push(block);
};}; if (ide && ide.stage) {if (ide.stage.globalBlocks.filter(definition => definition.category === category).length
) {blocks.push(this.subCategoryText((contains([category], category) ? localize(category[0].toUpperCase().concat(
category.slice(1))) : category).concat(localize('\'s Global Custom Blocks')))); blocks.push('=');}; ide.stage.globalBlocks.forEach(addCustomBlock
); if (this.customBlocks.filter(definition => definition.category === category).length) {if (ide.stage.globalBlocks.filter(definition =>
definition.category === category).length) {blocks.push('=');}; blocks.push(this.subCategoryText((contains([category],
category) ? localize(category[0].toUpperCase().concat(category.slice(1))) : category).concat(localize('\'s Local Custom Blocks')))); blocks.push('=');
};}; this.customBlocks.forEach(addCustomBlock); if (this.exemplar) {inheritedBlocks = this.inheritedBlocks(true); if (this.customBlocks.length &&
inheritedBlocks.length) {blocks.push('=');}; isInherited = true; inheritedBlocks.forEach(addCustomBlock);};} else {
var ide = this.parentThatIsA(IDE_Morph), blocks = [], isInherited = false, block, inheritedBlocks;
function addCustomBlock(definition) {if ((!definition.isHelper || includeHidden) && definition.category === category) {
block = definition.templateInstance(); if (isInherited) {block.ghost();}; blocks.push(block);
};}; if (ide && ide.stage) {if (ide.stage.globalBlocks.filter(definition => definition.category === category
).length + this.customBlocks.filter(definition => definition.category === category).length) {blocks.push('-');
}; if (ide.stage.globalBlocks.filter(definition => definition.category === category).length) {blocks.push(
this.subCategoryText((contains(SpriteMorph.prototype.categories, category) ? localize(category[0].toUpperCase().concat(
category.slice(1))) : category).concat(localize('\'s Global Custom Blocks')))); blocks.push('=');}; ide.stage.globalBlocks.forEach(addCustomBlock
); if (this.customBlocks.filter(definition => definition.category === category).length) {if (ide.stage.globalBlocks.filter(definition =>
definition.category === category).length) {blocks.push('=');}; blocks.push(this.subCategoryText((contains(SpriteMorph.prototype.categories,
category) ? localize(category[0].toUpperCase().concat(category.slice(1))) : category).concat(localize('\'s Local Custom Blocks')))); blocks.push('=');
};}; this.customBlocks.forEach(addCustomBlock); if (this.exemplar) {inheritedBlocks = this.inheritedBlocks(true); if (this.customBlocks.length &&
inheritedBlocks.length) {blocks.push('=');}; isInherited = true; inheritedBlocks.forEach(addCustomBlock);};}; return blocks;};

SpriteMorph.prototype.makeBlockButton = function () {var button = new PushButtonMorph(this, 'makeBlock', 'Make a block');
button.userMenu = this.helpMenu; button.selector = 'addCustomBlock'; button.showHelp = BlockMorph.prototype.showHelp; return button;};
...
button.userMenu = this.helpMenu; button.selector = 'addExtension'; button.showHelp = BlockMorph.prototype.showHelp; return button;}; /* Extensions. */

SpriteMorph.prototype.chooseExtension = function () {var extensionsMenu = new MenuMorph(world.childThatIsA(IDE_Morph),
((IDE_Morph.prototype.whiteExtensionsList.length === IDE_Morph.prototype.featureExtensions.length) ? 'All of the extensions are added.'
: 'Choose an extension...')); IDE_Morph.prototype.featureExtensions.forEach(function (ext) {if (!(contains(
IDE_Morph.prototype.whiteExtensionsList, ext[0]))) {extensionsMenu.addItem(ext[0], ext[1]);};});
extensionsMenu.popup(world, world.hand.bounds.origin); world.rerender();}; /* Extensions menu. */
...
SpriteMorph.prototype.freshPalette = function (category) {
    ...
    palette.userMenu = function () {
        ...
        menu.addItem(
            'rename a category...',
            () => this.parentThatIsA(IDE_Morph).renameUserCategory()
        );
        menu.addItem(
            'recolor a category...',
            () => this.parentThatIsA(IDE_Morph).recolorUserCategory()
        );
            menu.addItem(
                'delete a category...',
    ...
    if (category === 'unified') {
        ...
        blocks = SpriteMorph.prototype.allSortedCategories().reduce(
            (blocks, category) => {
                let header = [this.categoryText(category), '-'],
                    ...
                    showHeader = showCategories &&
                        !['motor', 'lists', 'other'].includes(category) &&
    ...
    } else if (category === 'other') {SpriteMorph.prototype.sortedCategories.forEach(
    cat => blocks.push(...this.customBlockTemplatesForCategory(
    cat)));}; /* View all the blocks from any category. :)  */
...
SpriteMorph.prototype.allPaletteBlocks = function () {
    ...
    var blocks = SpriteMorph.prototype.allSortedCategories().reduce(
        (blocks, category) => {
...
SpriteMorph.prototype.isDisablingBlock = function (aBlock) {
    ...
    }; if (contains(['motorOnAndWait', 'motorOn', 'motorOff', 'motorPower', 'motorDirection'], sel)) {
        return !IDE_Morph.showMotorBlocks;
    }; if (
...
SpriteMorph.prototype.changeBlockVisibility = function (aBlock, hideIt, quick) {
    ...
    if (aBlock.isCustomBlock) {
        ...
        this.variables.find(
            aBlock.blockSpec
        ).vars[aBlock.blockSpec].isHidden = asABool(hideIt);
    } else {
        if (hideIt) {
    ...
    return;}; dict = {
        blockComment: 'control',
        scriptChanger: 'control',
        reifyScript: 'operators',
        reifyReporter: 'operators',
        reifyPredicate: 'operators',
        doDeclareVariables: 'variables'
    };  cat = dict[aBlock.selector] || aBlock.category;
    if (cat === 'motor') {cat = 'motion';
    } else if (cat === 'events') {cat = 'control';
    } else if (cat === 'lists') {cat = 'variables';
...
SpriteMorph.prototype.blocksMatching = function (searchString,
strictly, types, varNames) {var blocks = [], blocksDict;
        search = searchString.toLowerCase();
        if (this.constructor.name === 'StageMorph') {var stage = this;
        } else {var stage = this.parentThatIsA(StageMorph);};
        var reporterized;

    if (!types || !types.length) {types = ['command', 'definitor',
    'hat', 'reporter', 'predicate', 'arrow', 'ring'];}; if (!varNames
    ) {varNames = [];}; function labelOf(aBlockSpec) {
        ...
        if (menu) {
            if (isString(menu)) {
                menu = (InputSlotMorph.prototype)[menu
                ](true);}; more = isNil(menu) ? null : (
                Object.values(menu)); if (!isNil(more
                )) {more = more.map(entry => {
...
SpriteMorph.prototype.searchBlocks = function (
    ...
    function showSelection() {
        ...
        return;};  focus = selection.outline(
            MorphicPreferences.isFlat ? new Color(
            150, 200, 255) : WHITE, 2);
        ...
        if (blocks.length && scriptFocus) {
            selection = blocks[0];
        };  searchPane.contents.children = [
        searchPane.contents.children[0
        ]]; blocks.forEach(block => {
            block.setPosition(new Point(x,
            y)); searchPane.addContents(
...
SpriteMorph.prototype.reporterize = function (expressionString) {var ast;
    ...
    function parseInfix(expression, operator, already) {
        ...
        monads = ['abs', 'neg', 'ceiling', 'floor', 'sqrt', 'sin', 'cos',
            'tan', 'asin', 'acos', 'atan', 'ln', 'log', 'lg', 'id'];
        alias = {
            ceil: 'ceiling',
            '!' : 'not'
        };
...
) {if (this.freqPlayer) {this.freqPlayer.stop();
this.freqPlayer = null; /* Reminder to stop. */};};
...
SpriteMorph.prototype.hideOnStage = function () {
    var stage = this.parentThatIsA(StageMorph);
    this.hide();
};
...
/*
    ...
    as icons in the corral. These can be deleted when are temporary and when
    the red stop button is pressed. Shallow-copying clones' scripts and costumes
...
//  Permanent Clones (BYOB 3.1.1 & Snavanced!)

SpriteMorph.prototype.clonify = function (stage, immediately) {if (immediately instanceof SpriteMorph) {var object = immediately;} else if ((isNil(immediately)) || ((new Process).inputOption(
immediately) === 'myself')) {var object = this;} else {var object = (world.children[0].stage.children.filter(child => (child.name === immediately)))[0];}; object.parts.forEach(part => part.clonify(
stage)); stage.cloneCount += 1; object.cloneOriginName = (object.isTemporary ? object.cloneOriginName : object.name); object.isTemporary = true; object.name = ''; stage.add(object); object.endWarp();};

SpriteMorph.prototype.spawn = function () {if (!this.isCorpse) {var myClone = this.fullCopy(true); myClone.gotoXY(this.xPosition(), this.yPosition()); (myClone
).clonify(this.parentThatIsA(StageMorph), myClone); myClone.perpetuate(); return myClone;} else {throw new Error('You can not clone a deleted sprite.');};};

// Temporary Clones (For compatibility)

SpriteMorph.prototype.createClone = function (immediately) {if (immediately instanceof SpriteMorph) {var object = immediately;} else if ((isNil(immediately)) || ((new Process).inputOption(immediately) ===
'myself')) {var object = this;} else {var object = (world.children[0].stage.children.filter(child => (child.name === immediately)))[0];}; var stage = this.parentThatIsA(StageMorph), clone;
if (object.isCorpse) {throw Error('cannot operate on a deleted sprite');}; if (stage && stage.cloneCount <= 5000) {clone = object.fullCopy(true); clone.clonify(stage, clone); clone.release();};
    clone.allHatBlocksFor('__clone__init__').forEach(block =>
...
SpriteMorph.prototype.newClone = function (immediately) {var clone = this.createClone(immediately
); if (isNil(clone)) {throw Error('Exceeding the maximum number of clones.');}; return clone;};
...
SpriteMorph.prototype.perpetuate = function () {var stage = this.parentThatIsA(StageMorph), ide = this.parentThatIsA(IDE_Morph); if (this.exemplar) {this.exemplar.perpetuate();}; if (!this.isTemporary || !stage
|| !ide) {return;}; this.isTemporary = false; this.name = ide.newSpriteName(this.cloneOriginName); this.cloneOriginName = ''; stage.cloneCount -= 1; ide.corral.addSprite(this); ide.sprites.add(this); this.parts
...
SpriteMorph.prototype.setColorDimension = function (idx, num) {
    var x = this.xPosition(), y = this.yPosition(), n = +num;

    idx = +idx;
    if (idx < 0 || idx > 3) {this.setSize(num);}
    if (idx === 0) {
...
) {if (this.parent) {this.parent.addBack(
this); this.changed();};};

SpriteMorph.prototype.goBack = function (layers) {
    var layer, newLayer = +layers, targetLayer;

    if (!this.parent) {return null;};
    layer = this.parent.children.indexOf(
...
SpriteMorph.prototype.doErase = function () {
    var stage = this.parentThatIsA(StageMorph),
    ...
    context.beginPath(); context.clearRect((
    trans.x - (extent.x / 2)), (trans.y - (
    extent.y / 2)), extent.x, extent.y);
    context.restore(); this.changed();
    stage.changed();
};
...
SpriteMorph.prototype.write = function (text, size) {
    ...
    if (text.split('\n').length > 0) {
    len = context.measureText(text.split('\n'
    )[text.split('\n').length - 1]).width;
    } else {len = context.measureText(text).width;};
    trans = trans.multiplyBy(1 / stage.scale);
...
SpriteMorph.prototype.fillOval = function (width, height) {
    var stage = this.parentThatIsA(StageMorph),
        context = stage.penTrails().getContext('2d'),
        rotation = radians(this.direction() - 90),
        trans = new Point(
    ...
    context.fillStyle = this.color.toString();
    trans = trans.multiplyBy(1 / stage.scale);
    context.beginPath(); context.translate(
    trans.x, trans.y); context.ellipse(0, 0,
    (asANum(width) / 2), (asANum(height) / 2),
    rotation, 0, (2 * Math.PI)); context.fill(
    ); context.translate(-trans.x, -trans.y
    ); context.restore(); this.changed();
    stage.changed();
};

SpriteMorph.prototype.fillRectangle = function (width, height, roundness) {
    var stage = this.parentThatIsA(StageMorph),
        context = stage.penTrails().getContext('2d'),
        rotation = radians(this.direction() - 90),
        trans = new Point(
        ...
        height = Math.max(height, 0);
        roundness = Math.max(roundness, 0);

    context.save();
    context.fillStyle = this.color.toString();
    trans = trans.multiplyBy(1 / stage.scale);
    context.beginPath(); context.translate(
    trans.x, trans.y); context.rotate(rotation);
    context.roundRect((asANum(width) / -2), (asANum(height) / -2), asANum(width),
    asANum(height), asANum(roundness / 2));
...
SpriteMorph.prototype.setScale = function (percentage, noShadow) {
    ...
    this.scale = Math.max(realScale, 0);
...
SpriteMorph.prototype.applyGraphicsEffects = function (canvas) {
  ...
  // Graphic effects from Scratch are completely based from ScratchPlugin.c
    ...
    function transform_negative (imagedata, value) {
        var pixels, i, l; value = (Math.max(Math.min(
        value, 100), 0) / 100); pixels = imagedata.data;
        for (i = 0, l = pixels.length; i < l; i += 4) {
            pixels[i] = pixels[i] + ((255 - (pixels[i] * 2)) * value);
            pixels[i + 1] = pixels[i + 1] + ((255 - (pixels[i + 1] * 2)) * value);
            pixels[i + 2] = pixels[i + 2] + ((255 - (pixels[i + 2] * 2)) * value);
        }
        return imagedata;
    }
    ...
    function transform_red (imagedata, value) {
        var pixels, i, l;
        pixels = imagedata.data;
        for (i = 0, l = pixels.length; i < l; i += 4) {if (value > 0) {
            pixels[i] = pixels[i] + Math.min(255, ((255 - pixels[i]) * (
            value / 100)));} else if (value < 0) {if (value < -100) {pixels [
            i] = pixels[i] + Math.min(255, ((255 - pixels[i]) * ((-100 - value
            ) / 100)));}; pixels[i + 1] = Math.max(0, pixels[i + 1] + (pixels[
            i + 1] * (value / 100))); pixels[i + 2] = Math.max(0, pixels[i + 2
            ] + (pixels[i + 2] * (value / 100)));}; /* Only the "red". :-) */
        }
        return imagedata;
    }

    function transform_green (imagedata, value) {
        var pixels, i, l;
        pixels = imagedata.data;
        for (i = 0, l = pixels.length; i < l; i += 4) {if (value > 0) {
            pixels[i + 1] = pixels[i + 1] + Math.min(255, ((255 - pixels[i + 1]) * (
            value / 100)));} else if (value < 0) {if (value < -100) {pixels [
            i + 1] = pixels[i + 1] + Math.min(255, ((255 - pixels[i + 1]) * ((-100 - value
            ) / 100)));}; pixels[i] = Math.max(0, pixels[i] + (pixels[
            i] * (value / 100))); pixels[i + 2] = Math.max(0, pixels[i + 2
            ] + (pixels[i + 2] * (value / 100)));}; /* Only the "green". :-) */
        }
        return imagedata;
    }

    function transform_blue (imagedata, value) {
        var pixels, i, l;
        pixels = imagedata.data;
        for (i = 0, l = pixels.length; i < l; i += 4) {if (value > 0) {
            pixels[i + 2] = pixels[i + 2] + Math.min(255, ((255 - pixels[i + 2]) * (
            value / 100)));} else if (value < 0) {if (value < -100) {pixels [
            i + 2] = pixels[i + 2] + Math.min(255, ((255 - pixels[i + 2]) * ((-100 - value
            ) / 100)));}; pixels[i] = Math.max(0, pixels[i] + (pixels[
            i] * (value / 100))); pixels[i + 1] = Math.max(0, pixels[i + 1
            ] + (pixels[i + 1] * (value / 100)));}; /* Only the "blue". :-) */
        }
        return imagedata;
    }
    ...
    if (this.graphicsChanged()) {
        ...
        if (Process.prototype.reportVariadicOr(new List([(this.graphicsValues['red'] > 0), (this.graphicsValues['green'] > 0), (this.graphicsValues['blue'] > 0)]
        )) || Process.prototype.reportOnlyOne([(this.graphicsValues['red'] < 0), (this.graphicsValues['green'] < 0), (this.graphicsValues['blue'] < 0)])) {
        if (this.graphicsValues.red) {
            imagedata = transform_red(
                imagedata,
                this.graphicsValues.red
            );
        }
        if (this.graphicsValues.green) {
            imagedata = transform_green(
                imagedata,
                this.graphicsValues.green
            );
        }
        if (this.graphicsValues.blue) {
            imagedata = transform_blue(
                imagedata,
                this.graphicsValues.blue
            );
        }
        }
...
SpriteMorph.prototype.bubble = function (data, isThought, isQuestion, isShouting, isWhispering) {
    var bubble,
        stage = this.parentThatIsA(StageMorph);
...
direction = Process.prototype.reportBasicRandom(0, 359999) / 1000;};
}; if (dist === 0 && this.isDown) {this.isDown = false; this.forward((
-dot / 2), optDir); this.isDown = true; this.forward(dot, optDir);
this.isDown = false; this.forward((-dot / 2), optDir); this.isDown = true;
return;} else if (dist >= 0) {dest = this.position().distanceAngle(dist,
direction);} else {dest = this.position().distanceAngle(Math.abs(
dist), (direction - 180));}; this.shadowAttribute('x position'
); this.shadowAttribute('y position'); this.setPosition(dest);
this.positionTalkBubble();}; /* The "forward" function is re-writed. */
...
SpriteMorph.prototype.faceToXY = function (x,
y) {this.setHeading(this.angleToXY(x, y));};
...
) {var stage = this.parentThatIsA(StageMorph), newX, newY, dest;
if (stage) {if (!noShadow) {this.shadowAttribute('x position');
this.shadowAttribute('y position');}; x = (!isFinite(+x) ? 0 : (
+x)); y = !isFinite(+y) ? 0 : +y; newX = stage.center().x + (
x * stage.scale); newY = (stage.center().y - (y * stage.scale));
if (this.costume) {dest = (new Point(newX, newY)).subtract((this
).rotationOffset);} else {dest = new Point(newX, newY).subtract(
this.extent().divideBy(2));}; this.setPosition(dest, justMe);
this.positionTalkBubble();};}; /* This is so important. */
...
SpriteMorph.prototype.setXPosition = (
function (num) {this.shadowAttribute(
'x position'); this.gotoXY(+num || 0,
this.yPosition(), false, true);});

SpriteMorph.prototype.changeXPosition = function (delta) {
this.setXPosition(this.xPosition() + (+delta || 0));};

SpriteMorph.prototype.setYPosition = (
function (num) {this.shadowAttribute(
'y position'); this.gotoXY(this.xPosition(
), +num || 0, false, true);});
...
SpriteMorph.prototype.bounceOffEdge = function () {
    ...
    if (!stage) {return null;};
    if (stage.bounds.containsRectangle(
    fb)) {return null;};
...
) {var delta, normal; if (!(this.costume)) {throw Error(('setting the'
) + ('rotation center requires a costume'));}; this.shadowAttribute(
'costumes'); delta = absoluteCoordinate.subtract(new Point((this
).xPosition(), this.yPosition())).divideBy(this.scale).rotateBy(
radians(90 - this.heading)); normal = (this.costume.rotationCenter
).add(new Point(delta.x, -delta.y)); (this.costume.rotationCenter
) = normal; this.changed(); this.fixLayout(); this.changed(
);}; SpriteMorph.prototype.moveRotationCenter = (
function () {world.activeHandle = new HandleMorph(
this, null, null, null, null, 'movePivot');});

SpriteMorph.prototype.setPivot = function (worldCoordinate
) {var stage = this.parentThatIsA(StageMorph), ide = (this
).parentThatIsA(IDE_Morph), cntr; if (stage) {cntr = (stage
).center(); this.setRotationCenter(new Point((((worldCoordinate
).x - cntr.x) / stage.scale), (cntr.y - worldCoordinate.y) / (
stage.scale)));  if (ide) {ide.recordUnsavedChanges();};};};
...
SpriteMorph.prototype.allMessageNames = function () {var msgs = [];
(this.allScripts()).forEach(script => {script.allChildren().forEach(
morph => {var txt; if ((morph instanceof InputSlotMorph) && ((morph
).choices && contains(['messagesMenu', 'messagesReceivedMenu'],
morph.choices))) {txt = morph.evaluate(); if (isString(txt) && (
txt !== '')) {if (!contains(msgs, txt)) {msgs.push(txt);};};};});
}); return msgs;};

SpriteMorph.prototype.allSendersOf = function (message, receiverName, known) {return (this.allScripts(
)).filter(script => script.isSending && script.isSending(message, receiverName, known));};

SpriteMorph.prototype.allHatBlocksFor = function (message) {if (typeof message === 'number') {message = message.toString();}; return this.scripts.children.filter(morph => {var sel = morph.selector, event;
if (sel) {if (sel === 'receiveMessage') {event = morph.inputs()[0].evaluate(); return ((event === message) || ((event instanceof Array) && (message !== '__shout__go__') && (message !== '__clone__init__')
));}; if (contains(['receiveGo', 'runScript'], sel)) {return message === '__shout__go__';} else if (sel === 'receiveOnClone') {return message === '__clone__init__';};}; return false;});};
    ...
    return this.scripts.children.filter(morph => {
        if (morph.selector) {
            if (morph.selector === 'receiveInteraction') {
                var choice = morph.inputs()[0].evaluate();
                return (Process.prototype.inputOption(choice) === interaction);
            };
        };  return false;
    });
};
...
SpriteMorph.prototype.mouseClickLeft = function () {return this.receiveUserInteraction('clicked');};

SpriteMorph.prototype.mouseEnter = function () {return this.receiveUserInteraction('mouse-entered');};

SpriteMorph.prototype.mouseDownLeft = function () {return this.receiveUserInteraction('pressed');};

SpriteMorph.prototype.mouseDownRight = function () {var ide = this.parentThatIsA(IDE_Morph);
if (ide && ide.isAppMode) {return this.receiveUserInteraction('pressed');} else {return;};};

SpriteMorph.prototype.mouseScroll = function (y) {return this.receiveUserInteraction('scrolled-' + ((y < 0) ? 'down' : 'up'));};
...
SpriteMorph.prototype.receiveUserInteraction = function (
    ...
    var stage = this.parentThatIsA(
    StageMorph), procs = [], hats;
...
) {if (this.isTemporary) {return;}; this.edit();};
...
SpriteMorph.prototype.getBatteryLevel = function () {return (isNil(world.batteryAPI) ? 0 : world.batteryAPI.level);};
SpriteMorph.prototype.getBatteryIsCharging = function () {return (isNil(world.batteryAPI) ? 0 : world.batteryAPI.charging);};

// SpriteMorph scenes

SpriteMorph.prototype.reportSceneName = function () {return (world.childThatIsA(IDE_Morph).scenes.at(this.reportSceneNumber()).name || localize('Untitled'));};
SpriteMorph.prototype.reportSceneNumber = function () {return world.childThatIsA(IDE_Morph).scenes.indexOf(world.childThatIsA(IDE_Morph).scene);};

// SpriteMorph loud?

SpriteMorph.prototype.reportLoud = function () {return ((world.children[0].stage.microphone.volume * 100) > 10);};
...
SpriteMorph.prototype.allBlockInstances = function (definition) {
    ...
    if (definition.isGlobal) {
        ...
        stage.globalBlocks.forEach(def => {
            ...
            if (def.body) {if (def.body.expression instanceof Morph
...
SpriteMorph.prototype.setExemplar = function (another, enableError) {
    ...
    if (contains(another.allExemplars(), this)) {
        if (enableError) {
            throw new Error(
    ...
    if (another instanceof SpriteMorph) {this.exemplar = another;};
    if (another) {
...
SpriteMorph.prototype.booleanMorph = function (
bool) {return BooleanImageMorph(bool);};
...
if (asABool(localStorage['-snap-setting-screenSize'])) {
StageMorph.prototype.dimensions = new Point(window.outerWidth / 2, window.outerHeight / 2);
} else {StageMorph.prototype.dimensions = new Point(480, 360);};
...
StageMorph.prototype.init = function (globals) {
    this.name = localize('THE NAME OF THE STAGE');
    ...
    this.cloneCount = 0;
    this.timerProcedure = new TimerFunction;
    this.tempo = 60;
    this.lastMessage = '';
    this.instrument = 1;
    this.volume = 100;
    ...
    this.getProjectionImage = null;
    this.stopProjectionSource = null;
...
StageMorph.prototype.render = function (ctx) {
    ...
    if (this.costume && !(this.costume.loaded instanceof Function)) {
        ...
        ctx.drawImage(
            ...
            Math.round((this.width() / this.scale - this.costume.width()) / 2),
            Math.round((this.height() / this.scale - this.costume.height()) / 2)
...
this.timerProcedure.pauseTime = Date.now(); this.timerProcedure.resumeTime = Process.prototype.reportBasicDifference(this.timerProcedure.pauseTime, Process.prototype.reportBasicProduct(
Process.prototype.reportBasicMax(0, value), 1000));};}; StageMorph.prototype.isTimerRunning = function () {return (this.timerProcedure.state === 'running');};
StageMorph.prototype.getTimer = function () {return Process.prototype.reportBasicQuotient(this.timerProcedure.getTime(), 1000);};

// StageMorph battery

StageMorph.prototype.getBatteryLevel = SpriteMorph.prototype.getBatteryLevel;
StageMorph.prototype.getBatteryIsCharging = SpriteMorph.prototype.getBatteryIsCharging;

// StageMorph scenes

StageMorph.prototype.reportSceneName = SpriteMorph.prototype.reportSceneName;
StageMorph.prototype.reportSceneNumber = SpriteMorph.prototype.reportSceneNumber;

// StageMorph loud?

StageMorph.prototype.reportLoud = SpriteMorph.prototype.reportLoud;

// StageMorph tempo

StageMorph.prototype.setTempo = function (bpm) {this.tempo = Math.max(20, (+bpm || 0));};
StageMorph.prototype.changeTempo = function (delta) {this.setTempo(this.getTempo() + (+delta || 0));};
StageMorph.prototype.getTempo = function () {return +this.tempo;};
...
StageMorph.prototype.step = function () {
    ...
    if (this.continuousProjection && this.projectionSource) {this.updateProjection();};
};
...
StageMorph.prototype.subCategoryText = SpriteMorph.prototype.subCategoryText;
StageMorph.prototype.blockTemplates = SpriteMorph.prototype.blockTemplates;
...
StageMorph.prototype.userMenu = function () {
    ...
    if (ide && ide.isAppMode) {return menu;};
    menu.addItem("edit", 'edit');
...
StageMorph.prototype.trailsLogAsSVG = function () {
    ...
    svg = '<svg xmlns="http://www.w3.org/2000/svg" ' +
        ...
        '>';
    svg += '<!-- Generated by Snavanced! - https://github.com/Alessandrito123/Snavanced -->';
...
StageMorph.prototype.hide = function () {this.isVisible = false; this.changed();};

StageMorph.prototype.show = function () {this.isVisible = true; this.changed();};

StageMorph.prototype.reportShown = SpriteMorph.prototype.reportShown;

// StageMorph cloning override

StageMorph.prototype.createClone = SpriteMorph.prototype.createClone;
StageMorph.prototype.newClone = SpriteMorph.prototype.newClone;
...
StageMorph.prototype.getBackgroundAttribute
    = SpriteMorph.prototype.getPenAttribute;
...
StageMorph.prototype.customPaletteButton = SpriteMorph.prototype.customPaletteButton;

StageMorph.prototype.chooseExtensionButton = SpriteMorph.prototype.chooseExtensionButton;

StageMorph.prototype.chooseExtension = SpriteMorph.prototype.chooseExtension;
...
StageMorph.prototype.noExtensionsText = SpriteMorph.prototype.noExtensionsText;
...
StageMorph.prototype.getBackgroundIdx = SpriteMorph.prototype.getCostumeIdx;

StageMorph.prototype.doSwitchToBackground = SpriteMorph.prototype.doSwitchToCostume;

StageMorph.prototype.doChangeToNextBackground = SpriteMorph.prototype.doWearNextCostume;
...
) {this.activeSounds.forEach(audio => (((typeof (audio
).stop) === 'function') ? audio.stop() : (((typeof (
audio.pause)) === 'function') ? audio.pause() : nop(
)))); this.activeSounds.forEach(audio => (isNil((audio
).remove) ? nop() : audio.remove())); if ((this.microphone
).modifier && this.microphone.isReady) {(this.microphone
).stop();}; this.children.forEach(child => isSnapObject(
child) ? child.stopFreq() : nop()); this.stopFreq();};
...
StageMorph.prototype.allSpecimens = StageMorph.prototype.specimens;

StageMorph.prototype.shadowAttribute = nop;

// StageMorph inheritance support - attributes

StageMorph.prototype.inheritsAttribute = (() => false);
...
StageMorph.prototype.inheritedVariableNames = (() => []);
...
StageMorph.prototype.inheritedBlocks = (() => []);
...
function SpriteBubbleMorph(data, stage, isThought, isQuestion, isShouting, isWhispering) {
    this.init(data, stage, isThought, isQuestion, isShouting, isWhispering);
}
...
SpriteBubbleMorph.prototype.init = function (
    ...
    this.isWhispering = isWhispering;
...
SpriteBubbleMorph.prototype.oldStep = SpriteBubbleMorph.prototype.step
SpriteBubbleMorph.prototype.step = function () {this.oldStep(); if (
this.isShouting) {this.changed(); this.fixLayout(); this.rerender();};};

// SpriteBubbleMorph contents formatting

SpriteBubbleMorph.prototype.dataAsMorph = function (data) {
    var contents, sprite = SpriteMorph.prototype,
        isText, img, scaledImg, width;
    if (data instanceof Morph) {
        if (isSnapObject(data)) {
            img = data.thumbnail(new Point(
            (40 * (asANum(this.isShouting) + 1)),
            (40 * (asANum(this.isShouting) + 1))));
            contents = new Morph;
        ...
        contents = new TextMorph(data,
            (sprite.bubbleFontSize * this.scale * (asANum(this.isShouting) + 1)),
            null, // fontStyle
            this.isShouting, // bold
            this.isWhispering, // italic
            'center' // position
        ...
        contents.selectForEdit = function () {
                var script = SpriteMorph.prototype.blockForSelector('getColor'),
                    prepare = script.prepareToBeGrabbed,
                    ide = (this.parentThatIsA(IDE_Morph) ||
                    this.world().childThatIsA(IDE_Morph));
                script.children[1].setColor(data.copy());
                script.isDraggable = true;

                script.prepareToBeGrabbed = function (
                    hand) {prepare.call(this, hand);
                    hand.grabOrigin = {
                        origin: ide.palette,
        ...
        img = BooleanImageMorph(data).fullImage();
        contents = new Morph;
        contents.isCachingImage = true;
        contents.bounds.setWidth(img.width);
        contents.bounds.setHeight(img.height);
        ...
        contents.selectForEdit = function () {
                var script = SpriteMorph.prototype.blockForSelector('reportBoolean'),
                    ...
                    ide = this.parentThatIsA(IDE_Morph) ||
                        this.world().childThatIsA(IDE_Morph);
                script.children[0].setContents(data);
                script.isDraggable = true;
        ...
        contents = new TextMorph((isNil(data) ? '' : (isNil(
            data.textRepresentation) ? data.toString() : getText(
            data.textRepresentation, data))), (sprite.bubbleFontSize * (
            this.scale * (asANum(this.isShouting) + 1))),
            null, // fontStyle
            !this.isShouting, // bold
            this.isWhispering, // italic
            'center' // position
        ...
        width = Math.max(
            ...
            sprite.bubbleMaxTextWidth * (asANum(this.isShouting) + 1) * this.scale
            );
...
SpriteBubbleMorph.prototype.setScale = function (
scale) {this.scale = scale; this.changed();
...
SpriteBubbleMorph.prototype.fixLayout = function () {
    ...
    )); this.normalExtent = this.extent().divideBy(this.scale);
};
...
Costume.prototype.edit = function (aWorld, anIDE, isnew, oncancel, onsubmit, isReturned) {
...
Costume.prototype.rasterized = function () {return this;};
...
Costume.prototype.pixels = function (
    ...
    if (asABool(localStorage['-snap-setting-backToOldPixelGetting'])) {
...
function CostumeEditorMorph(costume) {this.init(costume);};
...
Note.prototype.fadeIn = new Float32Array(
2); Note.prototype.fadeIn[0] = [0.0];
Note.prototype.fadeIn[1] = [1.0];
Note.prototype.fadeOut = new Float32Array(
2); Note.prototype.fadeOut[0] = [1.0];
...
Note.prototype.play = function (type, gainNode, pannerNode) {
    ...
    this.oscillator.frequency.value = isNil(this.frequency) ?
        Math.pow(2, (this.pitch - 59.760344731969152) / 12) * 256 : this.frequency;
...
Note.prototype.pause = function () {this.stop();};
...
Microphone.prototype.stepAudio = function (event) {
    ...
    if (this.pitch > 0) {
        this.note = (
            12 * (Math.log(this.pitch / 256) / Math.log(2))
        ) + 59.760344731969152;
    }; this.isReady = true;
    this.isStarted = false;
};
...
Microphone.prototype.detectPitchAndVolume = function (buf, sampleRate) {
    ...
    var SIZE = buf.length,
        ...
        best_offset = -1, best_correlation = 0,
        rms = 0, foundGoodCorrelation = false,
        correlations = this.correlations,
        channels = this.outChannels.length,
        correlation, lastCorrelation,
        offset, shift, i, k, val,
        modified;
        ...
        if (this.contents instanceof Morph) {
            ...
            this.contentsMorph.selectForEdit = function () {
                var script = SpriteMorph.prototype.blockForSelector('getColor'),
                    ...
                    ide = this.parentThatIsA(IDE_Morph) ||
                        this.world().childThatIsA(IDE_Morph);
                script.children[1].setColor(myself.contents.copy());
                script.isDraggable = true;
            ...
            img = BooleanImageMorph(this.contents).fullImage();
            this.contentsMorph = new Morph;
            this.contentsMorph.isCachingImage = true;
            this.contentsMorph.bounds.setWidth(img.width);
            this.contentsMorph.bounds.setHeight(img.height);
            ...
            this.contentsMorph.selectForEdit = function () {
                var script = SpriteMorph.prototype.blockForSelector('reportBoolean'),
                    ...
                    ide = this.parentThatIsA(IDE_Morph) ||
                        this.world().childThatIsA(IDE_Morph);
                script.children[0].setContents(myself.contents);
                script.isDraggable = true;
            ...
            this.contentsMorph.selectForEdit = function () {
                var script = myself.contents.beDraggable(),
                    prepare = script.prepareToBeGrabbed,
                    ide = this.parentThatIsA(IDE_Morph) ||
                        this.world().childThatIsA(IDE_Morph);
            ...
            this.contentsMorph = new TextMorph(
                (isNil(this.contents) ? '' : ((
                this.contents.textRepresentation === undefined
                ) ? this.contents.toString() : getText(
                this.contents.textRepresentation, this.contents
                ))), fontSize, null, true, false, 'center'
            );  if (this.isEditable && (isNil(
                this.contents.textRepresentation
                ) || !isNil(this.contents.enableEditing))
                ) {this.contentsMorph.isEditable = true;
                this.contentsMorph.enableSelecting();
            };  this.contentsMorph.setColor(WHITE);
        };  this.add(this.contentsMorph);
...
WatcherMorph.prototype.update = function () {
    ...
    if (this.target && this.getter) {
        ...
        if (newValue !== '' && !isNil(newValue)) {
            num = +newValue;
            if (this.getter === 'getTimer') {
                newValue = asABool(localStorage['-snap-setting-betterTimer']) ? TimerFunction.prototype.toCronoString(num) : num;
            } else if (typeof newValue !== 'boolean' && !isNaN(num)) {
...
function StagePrompterMorph(question) {this.init(question);};
...
StagePrompterMorph.prototype.init = function (question) {
    ...
    // additional properties
    this.isDone = false;
    if (isNil(question) ? '' : ((question.toString
    instanceof Function) ? question.toString() : '')) {
...
this.inputField.edit();}; (StagePrompterMorph.prototype
).accept = function () {this.isDone = true;};
...
function Project (scenes, current) {var projectScene; this.scenes = (scenes || new List);
this.currentScene = current; this.name = null; this.notes = null; this.thumbnail = null;
projectScene = this.scenes.at(1); if (projectScene) {this.name = projectScene.name; (this
).notes = projectScene.notes; this.thumbnail = normalizeCanvas(projectScene.stage.thumbnail(
SnapSerializer.prototype.thumbnailSize));}; this.sceneIdx = null; this.trash = [];}; (Project
).prototype.initialize = function () {this.currentScene = this.scenes.at(+(this.sceneIdx) || 1
); return this;}; Project.prototype.addDefaultScene = function () {var scene = new Scene;
scene.addDefaultSprite(); this.scenes.add(scene);};
...
Scene.prototype.updateTrash = function () {this.trash = this.trash.filter(sprite => sprite.isCorpse);};

Scene.prototype.restart = function () {
    var ide; window.speechSynthesis.cancel();
    this.stage.threads.pauseCustomHatBlocks = false;
    this.stage.stopAllActiveSounds(); (this.stage
    ).clearEffects(); (this.stage.threads
    ).resumeAll(this.stage); this.stage.keysPressed = {
    }; this.stage.threads.stopAll(); (this.stage
    ).timerProcedure.stopNow(); world.children.filter(
    function (child) {if (child instanceof DialogBoxMorph
    ) {return (child.key).includes('debug') && contains(
    this.stage.threads.processes, child.process);} else {
    return false;};}).forEach(function (child) {child.destroy(
...
Scene.prototype.stop = function (forGood) {
    ...
    ).clearEffects(); this.stage.threads.resumeAll(
    this.stage); this.stage.keysPressed = {
    ...
    this.stage.stopProjection();}; (this.stage
    ).children.forEach(morph => {if ((morph
    ).stopTalking) {morph.stopTalking();
    }; if (morph.clearEffects) {(morph
    ).clearEffects();};}); (this.stage
    ).removeAllClones(); ide = (this.stage
    ).parentThatIsA(IDE_Morph); if (ide) {
    ide.controlBar.pauseButton.refresh();
    ide.controlBar.stopButton.refresh();};
