...
/*
    ...
    asXML()                 - answer a xml-formatted String of myself
...
function List (array) {this.type = null; /* for UI lists, such as costumes, sounds, sprites */
this.contents = array || []; this.first = null; this.rest = null; this.isLinked = false;
this.lastChanged = Date.now(); /* for updating lists and show the Morphs too faster */};
...
List.prototype.slice = function (indices) {
    ...
    //
    // negative indices are used in exercises and
...
List.prototype.asJSON = function () {
    ...
    function objectify(list) {
        ...
        return items.map(element => element instanceof List ?
            objectify(element) : element instanceof Color ?
...
List.prototype.canBeTXT = function () {return this.itemsArray().every(item => isString(item) || (typeof item === 'number'));};

List.prototype.asTXT = function () {return this.itemsArray().join('\n');};
...
List.prototype.canBeXML = function () {
    return this.itemsArray().every(value => {
        return !isNaN(+value) ||
            isString(value) ||
            contains([true, false], value) ||
...
List.prototype.canBeBlocks = function () {
    return this.itemsArray().every(value => {
        return !isNaN(+value) ||
            isString(value) ||
            contains([true, false], value) ||
            ...
            (value instanceof List && value.canBeBlocks());
    });
};
...
List.prototype.blockify = function (limit = 500, count = [0]) {var block = SpriteMorph.prototype.blockForSelector('reportNewList'), slots = block.inputs()[0], len = this.length(), bool, i, value;
block.isDraggable = true; slots.removeInput(); for (i = 0; i < len && count[0] < limit; i += 1) {value = this.at(i + 1); if (value instanceof List) {slots.replaceInput(slots.addInput(), value.blockify(limit, count
));} else if (typeof value === 'boolean') {var bool = SpriteMorph.prototype.blockForSelector('reportBoolean'); bool.inputs()[0].setContents(value); bool.isDraggable = true; slots.replaceInput(slots.addInput(),
bool);} else if (value instanceof Color) {var clr = SpriteMorph.prototype.blockForSelector('getColor'); clr.inputs()[0].color = value; clr.isDraggable = true; slots.replaceInput(slots.addInput(), clr);} else if (
value instanceof Context) {var context = value.beDraggable().fullCopy(); slots.replaceInput(slots.addInput(), context);} else {slots.addInput(value.toString());}; count[0] += 1;}; slots.fixBlockColor(null, true
); return block;}; /* Blockifies your selected list, blockify is a function to convert to blocks a list with first class types and base blocks. Try it, is now revamped with a new cool look!!! Woohoo!!! :D */
...
/* ListWatcherMorph inherits from BoxMorph: */ ListWatcherMorph.prototype = new BoxMorph;
...
function ListWatcherMorph(list, parentCell) {this.init(list, parentCell);};

ListWatcherMorph.prototype.init = function (list, parentCell) {var myself = this, readOnly; this.list = list || new List; this.start = 1; this.range = 100; this.lastUpdated = 0;
this.lastCell = null; this.parentCell = parentCell || null; this.label = new StringMorph(localize('length: ') + this.list.length(), SyntaxElementMorph.prototype.fontSize, null,
false, false, false, MorphicPreferences.isFlat ? ZERO : new Point(1, 1), WHITE); this.label.mouseClickLeft = function () {myself.startIndexMenu();}; this.frame = new ScrollFrameMorph(
null, 10); this.frame.alpha = 0; this.frame.acceptsDrops = false; this.frame.contents.acceptsDrops = false; this.handle = new HandleMorph(this, 80, 70, 3, 3); this.handle.setExtent(
new Point(13, 13)); this.arrow = new ArrowMorph('down', SyntaxElementMorph.prototype.fontSize); this.arrow.mouseClickLeft = (() => myself.startIndexMenu()); this.arrow.setRight(
this.handle.right()); this.arrow.setBottom(this.handle.top()); this.handle.add(this.arrow); readOnly = this.list.type && !contains(['text', 'number'], this.list.type); if (
readOnly) {this.plusButton = null;} else {this.plusButton = new PushButtonMorph(this.list, 'add', '+'); this.plusButton.padding = 0; this.plusButton.edge = 0;
this.plusButton.outlineColor = this.color; this.plusButton.fixLayout();}; ListWatcherMorph.uber.init.call(this, SyntaxElementMorph.prototype.rounding, 1, new Color(120, 120, 120));
this.color = new Color(220, 220, 220); this.isDraggable = false; this.setExtent(new Point(80, 70).multiplyBy(SyntaxElementMorph.prototype.scale)); this.add(this.label); this.add(
this.frame); if (!readOnly) {this.add(this.plusButton);}; this.add(this.handle); this.handle.fixLayout(); this.update(); this.fixLayout(); this.cursorStyle = 'default';};
    ...
    if (this.list.canBeJSON()) {
        menu.addItem(
            ...
            () => {
                ...
                } else if (this.list.canBeXML()) {
                    ide.saveFileAs(
                        this.list.asXML(),
                        'text/xml;charset=utf-8',
                        localize('data') // name
                    );
                } else if (this.list.canBeJSON()) {
                    ide.saveFileAs(
                        this.list.asJSON(),
...
ListWatcherMorph.prototype.mouseDoubleClick = function (pos) {if (List.prototype.enableTables) {new TableDialogMorph(this.list).popUp(this.world());} else {this.escalateEvent('mouseDoubleClick', pos);}};

// ListWatcherMorph hiding/showing:

ListWatcherMorph.prototype.show = function () {ListWatcherMorph.uber.show.call(this); this.frame.contents.adjustBounds();};
