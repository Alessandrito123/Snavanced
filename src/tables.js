/*
    ...
    prerequisites:
    --------------
    needs morphic.js, list.js, widgets.js, byob.js, threads.js
...
TableCellMorph.prototype.render = function (ctx) {
    var dta = this.labelString || this.dataRepresentation(this.data),
        raw = this.getData(), fontSize = SyntaxElementMorph.prototype.fontSize,
        ...
        width = this.width(), height = this.height(), txtWidth, txtHeight, x, y;

    this.isDraggable = (raw instanceof Context) || (raw instanceof Costume) ||
        (raw instanceof Sound) || (raw instanceof Color) || ((typeof raw) === 'boolean');
    if (this.isDraggable) {
...
TableCellMorph.prototype.dataRepresentation = function (dta) {
    ...
    if (dta instanceof Morph) {
        ...
        return BooleanImageMorph(dta).fullImage();
    } else if (dta instanceof Color) {
        return ColorImageMorph(dta.copy()).fullImage();
...
TableCellMorph.prototype.selectForEdit = function () {
    ...
    if (this.getData() instanceof Color) {
        return this.selectColorForEdit();
    };
    if ((typeof this.getData()) === 'boolean') {
        return this.selectBooleanForEdit();
    };
};

TableCellMorph.prototype.selectContextForEdit = function () {
    var script = this.getData().beDraggable(),
...
TableCellMorph.prototype.selectColorForEdit = function () {
    var script = SpriteMorph.prototype.blockForSelector('getColor'),
        prepare = script.prepareToBeGrabbed,
        ide = this.parentThatIsA(IDE_Morph) ||
            this.world().childThatIsA(IDE_Morph);
        script.children[1].setColor(this.getData().copy());
...
TableCellMorph.prototype.selectBooleanForEdit = function () {
    var script = SpriteMorph.prototype.blockForSelector('reportBoolean'),
        prepare = script.prepareToBeGrabbed,
        ide = this.parentThatIsA(IDE_Morph) ||
            this.world().childThatIsA(IDE_Morph);
        script.children[0].setContents(this.getData());
        script.isDraggable = true;
...
TableMorph.prototype.userMenu = function () {
    ...
    if (this.parentThatIsA(TableDialogMorph)) {
        ...
        if (this.table instanceof List && this.table.canBeBlocks()) {
            menu.addItem(
                'blockify',
                () => {
        ...
        if (this.table instanceof List && this.table.canBeJSON()) {
            menu.addItem(
                ...
                () => {
                    ...
                    } else if (this.table.canBeXML()) {
                        ide.saveFileAs(
                            this.table.asXML(),
                            'text/xml;charset=utf-8',
                            localize('data') // name
                        );
                    } else if (this.table.canBeJSON()) {
                        ide.saveFileAs(
                            this.table.asJSON(),
